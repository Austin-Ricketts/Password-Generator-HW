...
function generatePassword() {
  ...
  // replace passLength with lengthChoice
  if (passLength < min) {
    ...
    if (response4) {
      ...
      var stringPassword = "";
      for (v in userChoice) {
        stringPassword += userChoice[v];
      }
    //create a for loop that starts at passLength
    for (var i = 0; i < passLength - userChoice.length; i++) {
      console.log(passLength);
      stringPassword += (Math.floor(Math.random(passLength) * 10));
    }
